...
namespace Top.Conversion.Gltf.Tests
{
    public class GltfBuilderTests
    {
        ...
        public void An_animation_that_never_got_a_channel_is_dropped()
        {
            ...
            var input = b.Buffer.AddScalars([0f, 1f], withMinMax: true);
        ...
        [Test]
        public void Extras_land_on_the_element_the_ref_stands_for()
        {
            var b = new GltfBuilder("scene");
            var payload = new JObject { ["anything"] = "at all" };

            var node = b.AddNode("node").WithExtras(payload);
            var mesh = b.AddMesh("mesh").WithExtras(payload);
            var material = b.AddMaterial("mat").WithExtras(payload);
            var texture = b.AddTexture("t.png").WithExtras(payload);
            var animation = b.AddAnimation("clip").WithExtras(payload);
            var skin = b.AddSkin("skin", [node], 0).WithExtras(payload);

            var doc = b.Document;

            Assert.That(doc.Nodes[node.Index].Extras, Is.SameAs(payload));
            Assert.That(doc.Meshes[mesh.Index].Extras, Is.SameAs(payload));
            Assert.That(doc.Materials[material.Index].Extras, Is.SameAs(payload));
            Assert.That(doc.Textures[texture.Index].Extras, Is.SameAs(payload));
            Assert.That(doc.Animations[animation.Index].Extras, Is.SameAs(payload));
            Assert.That(doc.Skins[skin.Index].Extras, Is.SameAs(payload));
        }
        ...
        public void Primitives_bind_their_mesh_material_and_accessors()
        {
            ...
            var indices = b.Buffer.AddIndices([0, 1, 2]);
