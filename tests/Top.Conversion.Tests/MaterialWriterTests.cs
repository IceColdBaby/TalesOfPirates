...
namespace Top.Conversion.Tests
{
    public class MaterialWriterTests
    {
        ...
        private static MaterialExtras Read(GltfMaterial material)
        {
            MaterialExtras.TryRead(material.Extras as JObject, out var extras);
        ...
        private static void AssertSurvivesTheFile(RenderState state)
        {
            ...
            builder.AddMaterial("mat").WithExtras(new JObject
            {
                [MaterialExtras.Key] = MaterialWriter.Extras(Object(), 0, state, _ => 0).ToJson(),
            });
            ...
            var read = Read(GltfReader.Read(stream).Document.Materials[0]).RenderState;
        ...
        public void Every_emitted_material_carries_its_payload_and_nothing_else_does()
        {
            ...
            {
                Assert.That(Read(material), Is.Not.Null, material.Name);
            }
        ...
        public void Flipbook_frames_index_textures_the_document_holds()
        {
            ...
            var extras = Read(doc.Materials[0]);
