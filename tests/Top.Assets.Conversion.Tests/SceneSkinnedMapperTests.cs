...
{
    public class SceneSkinnedMapperTests
    {
        ...
        private static SceneModel MakeModel(bool skinned = true)
        {
            ...
            {
                Version = 4100,
                GeometryObjects = new[] { stat, animated },
                Helpers = new Helper[0],
            };
        }
        ...
        {
            var conversion = GeometryObjectGltfMapper.Map(MakeModel(), "prop");
        ...
        public void Object_with_both_bone_and_matrix_animation_emits_only_the_bone_clip()
        {
            ...
            {
                Frames = new[] { Matrix4x4.Identity, Matrix4x4.CreateTranslation(1f, 0f, 0f) },
            };

            var conversion = GeometryObjectGltfMapper.Map(model, "prop");
            var doc = conversion.Document;
            ...
            Assert.That(clips[0].Channels.Select(c => c.Target.Node.Value).Distinct().OrderBy(n => n),
                Is.EqualTo(joints.OrderBy(n => n)));
            Assert.That(conversion.Warnings, Has.Some.Contains(
                "object 1 has both bone and matrix animation; the matrix animation is ignored"));
        }

        [Test]
        public void Bone_dummies_hang_off_their_bone()
        {
            var doc = GeometryObjectGltfMapper.Map(MakeModel(), "prop").Document;
        ...
        [Test]
        public void Local_matrix_on_a_skinned_object_is_ignored_with_a_warning()
        {
            var model = MakeModel();
            model.GeometryObjects[1].LocalMatrix = Matrix4x4.CreateTranslation(5f, 0f, 0f);

            var conversion = GeometryObjectGltfMapper.Map(model, "prop");
            var meshNode = conversion.Document.Nodes.Find(n => n.Name == "geom_1");

            Assert.That(meshNode.Matrix, Is.Null);
            Assert.That(meshNode.Translation, Is.Null);
            Assert.That(conversion.Warnings, Has.Some.Contains("local matrix is ignored"));
        }

        [Test]
        public void Parent_of_a_skinned_object_is_ignored_with_a_warning()
        {
            var model = MakeModel();
            ...
            Assert.That(conversion.Warnings, Has.Some.Contains("its parent 0 is ignored"));
        }

        [Test]
        public void Blend_data_without_bone_animation_still_warns()
        {
            var conversion = GeometryObjectGltfMapper.Map(MakeModel(skinned: false), "prop");

            Assert.That(conversion.Document.Skins, Is.Null);
            Assert.That(conversion.Document.Animations, Is.Null);
            Assert.That(conversion.Warnings, Has.Some.Contains("skinning data on object 1 ignored"));
        }
        ...
        public void Converts_by_bd005_with_its_embedded_skeleton()
        {
            ...
            var conversion = GeometryObjectGltfMapper.Map(model, "by-bd005");
            var doc = conversion.Document;
            var data = new GltfData(new GltfFile { Document = doc, BinChunk = conversion.Bin });
            var container = doc.Nodes.FindIndex(n => n.Name == "geom_4_skeleton");

            // geom_2 ships three zero-length normals.
            Assert.That(conversion.Warnings,
                Is.EqualTo(new[] { "object 2 has 3 degenerate normals; replaced" }));
            Assert.That(doc.Skins, Has.Count.EqualTo(1));
