...
namespace Top.Assets.Conversion.Tests
{
    public class RigMapperTests
    {
        private static BoneAnimation MakeSkeleton()
        {
            return new BoneAnimation
            {
                ...
                Tracks = new BoneTrack[]
                {
                    ...
                    {
                        Positions = new[] { Vector3.Zero, Vector3.Zero },
                        Rotations = new[] { Quaternion.Identity, Quaternion.Identity },
                    },
                    new QuaternionBoneTrack
                    {
                        Positions = new[] { new Vector3(0f, 0f, 1f), new Vector3(0f, 0f, 2f) },
                        Rotations = new[] { Quaternion.Identity, Quaternion.Identity },
                    },
                },
            };
        ...
        {
            var conversion = RigGltfMapper.Map(MakeSkeleton(),
                new[] { Action(1, 0, 1), Action(60, 1, 1) }, "rig");
        ...
        {
            var conversion = RigGltfMapper.Map(MakeSkeleton(), new[] { Action(5, 1, 1) }, "rig");
            var data = new GltfData(new GltfFile
            {
                Document = conversion.Document,
                BinChunk = conversion.Bin,
            });

            var times = data.ReadFloats(conversion.Document.Animations[0].Samplers[0].Input);
        ...
        public void Single_position_quaternion_tracks_hold_the_position()
        {
            ...
            ((QuaternionBoneTrack)skeleton.Tracks[1]).Positions =
                new[] { new Vector3(0f, 0f, 3f) };

            var conversion = RigGltfMapper.Map(skeleton, new[] { Action(1, 0, 1) }, "rig");
            var doc = conversion.Document;
            var data = new GltfData(new GltfFile { Document = doc, BinChunk = conversion.Bin });
            var channel = doc.Animations[0].Channels
                .First(c => c.Target.Node == 1 && c.Target.Path == "translation");
        ...
        public void Clips_without_usable_tracks_are_skipped()
        {
            ...
            Assert.That(conversion.Warnings, Has.Some.Contains("no usable tracks"));
        }
        ...
        public void Matrix_fallback_bones_get_no_channels()
        {
            ...
            Assert.That(conversion.Warnings, Has.Some.Contains("not TRS-decomposable"));
        ...
        public void Matrix_tracks_decompose_to_channels()
        {
            ...
            skeleton.Tracks = new BoneTrack[]
            {
                new MatrixBoneTrack
                {
                    Frames = new[] { Matrix4x4.Identity, Matrix4x4.Identity },
                },
                new MatrixBoneTrack
                {
                    Frames = new[]
        ...
        {
            var skeleton = MakeSkeleton();
            skeleton.Tracks = new[] { skeleton.Tracks[0] };

            var conversion = RigGltfMapper.Map(skeleton, new[] { Action(1, 0, 1) }, "rig");
            var doc = conversion.Document;

            Assert.That(conversion.Warnings, Has.Some.Contains("child has no track"));
            Assert.That(doc.Animations[0].Channels.Any(c => c.Target.Node == 1), Is.False);
        ...
        public void Tracks_shorter_than_the_clip_warn()
        {
            var skeleton = MakeSkeleton();
            var track = (QuaternionBoneTrack)skeleton.Tracks[1];
            track.Positions = new[] { Vector3.Zero };
            track.Rotations = new[] { Quaternion.Identity };

            var conversion = RigGltfMapper.Map(skeleton, new[] { Action(1, 0, 1) }, "rig");
            var doc = conversion.Document;

            Assert.That(conversion.Warnings, Has.Some.Contains("shorter than the timeline"));
            Assert.That(doc.Animations[0].Channels.Any(c => c.Target.Node == 1), Is.False);
        }
        ...
        {
            var conversion = RigGltfMapper.Map(MakeSkeleton(), new[] { Action(1, 0, 1) }, "rig");
            var doc = conversion.Document;
        ...
        {
            var conversion = RigGltfMapper.Map(MakeSkeleton(), null, "rig");

            Assert.That(conversion.Document.Animations.Single().Name, Is.EqualTo("rig_timeline"));
            Assert.That(conversion.Warnings, Has.Some.Contains("full timeline"));
        }

        [Test]
        public void Out_of_range_clips_clamp_and_warn()
        {
            var conversion = RigGltfMapper.Map(MakeSkeleton(), new[] { Action(1, 0, 9) }, "rig");
            var data = new GltfData(new GltfFile
            {
                Document = conversion.Document,
                BinChunk = conversion.Bin,
            ...
            Assert.That(conversion.Warnings, Has.Some.Contains("clamped"));
        }
        ...
        public void Converts_real_rig_with_action_table()
        {
            ...
            var conversion = RigGltfMapper.Map(skeleton, actions, "0001");
            var doc = conversion.Document;
        ...
        [Test]
        public void ConvertRig_writes_a_glb()
        {
            var outputDir = Path.Combine(Path.GetTempPath(), "top-rig-tests");

            try
            {
                var result = ModelConversion.ConvertRig(MakeSkeleton(),
                    new[] { Action(1, 0, 1) }, "rig", outputDir);
