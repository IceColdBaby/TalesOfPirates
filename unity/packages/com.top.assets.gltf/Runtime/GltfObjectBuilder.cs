...
namespace Top.Assets.Gltf
{
    /// <summary>
    /// Builds the Unity hierarchy a glTF document describes: nodes, meshes, skins
    /// and legacy animation clips attached to the nodes that host them.
    /// </summary>
    public static class GltfObjectBuilder
    {
        public static GameObject Build(GltfData data, string name,
            IReadOnlyList<Material> materials, Material fallback)
        {
            ...
            if (doc.Nodes != null)
            {
                var objects = BuildHierarchy(doc, root, meshes, materials, fallback);
                BindSkins(data, objects, meshes, materials, fallback);
                BuildClips(data, root, objects);
            }

            return root;
        }

        private static GameObject[] BuildHierarchy(GltfDocument doc, GameObject root,
            Mesh[] meshes, IReadOnlyList<Material> materials, Material fallback)
        {
            ...
            for (var i = 0; i < doc.Nodes.Count; i++)
            {
                ...
                if (node.Mesh != null && node.Skin == null)
                {
                    ...
                    if (!Naming.IsHelper(objects[i].name))
                    {
                        objects[i].AddComponent<MeshRenderer>().sharedMaterials =
                            MaterialsOf(doc, node.Mesh.Value, materials, fallback);
                    }
                }
            }
        ...
        private static void BindSkins(GltfData data, GameObject[] objects, Mesh[] meshes,
            IReadOnlyList<Material> materials, Material fallback)
        {
            var doc = data.Document;
            ...
            for (var i = 0; i < doc.Nodes.Count; i++)
            {
                ...
                renderer.sharedMaterials = MaterialsOf(doc, node.Mesh.Value, materials, fallback);
            }
        }

        private static void BuildClips(GltfData data, GameObject root, GameObject[] objects)
        {
            var doc = data.Document;
        ...
        private static Mesh BuildMesh(GltfData data, int meshIndex)
        {
            ...
            foreach (var primitive in gltfMesh.Primitives)
            {
                ...
                if (primitive.Attributes.TryGetValue("COLOR_0", out var colorAccessor))
                {
                    ...
                    var components = GltfData.ComponentCount(data.Document.Accessors[colorAccessor].Type);
        ...
        private static void ApplyTransform(Transform transform, GltfNode node)
        {
            ...
            {
                transform.localRotation = new Quaternion(-node.Rotation[0], -node.Rotation[1], node.Rotation[2],
                    node.Rotation[3]);
            }
        ...
        private static Material[] MaterialsOf(GltfDocument doc, int meshIndex,
            IReadOnlyList<Material> materials, Material fallback)
        {
            var primitives = doc.Meshes[meshIndex].Primitives;
            var bound = new Material[primitives.Count];

            for (var i = 0; i < primitives.Count; i++)
            {
                var index = primitives[i].Material;

                bound[i] = index != null ? materials[index.Value] : fallback;
            }
            ...
            Transform host, GameObject[] objects)
        {
            var clip = new AnimationClip
            {
                ...
                frameRate = AnimationRate.FramesPerSecond,
            };
            ...
            float[] times, float[] values, bool negateZ)
        {
            ...
            for (var i = 0; i < times.Length; i++)
            {
                ...
                z[i] = new Keyframe(times[i], negateZ ? -values[(i * 3) + 2] : values[(i * 3) + 2]);
            }
