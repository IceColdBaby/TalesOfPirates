...
namespace Top.Engine.Editor.Tests
{
    public class OpacitySamplingTests
    {
        ...
        {
            // hdjd-cbd01 subset 2 ships 0:1 24:0.3 46:1 71:0.2 100:1.
            var track = Track(new[] { 0, 24, 46, 71, 100 },
                new[] { 1f, 0.3f, 1f, 0.2f, 1f });

            Assert.That(OpacityAnimation.Sample(track, 24f), Is.EqualTo(0.3f));
            Assert.That(OpacityAnimation.Sample(track, 35f),
                Is.EqualTo(Mathf.Lerp(0.3f, 1f, 11f / 22f)).Within(1e-5f));
        }
        ...
        {
            // The earlier duplicate's value wins at the shared frame; the
            // later one opens the next segment. No division by zero occurs
            // because the scan never enters a zero-width segment.
            var track = Track(new[] { 0, 50, 50, 100 }, new[] { 1f, 0.2f, 0.8f, 1f });

            Assert.That(OpacityAnimation.Sample(track, 50f), Is.EqualTo(0.2f));
            Assert.That(OpacityAnimation.Sample(track, 60f),
                Is.EqualTo(Mathf.Lerp(0.8f, 1f, 0.2f)).Within(1e-5f));
        }
    }
}
