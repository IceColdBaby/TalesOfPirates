...
namespace Top.Engine.Editor.Tests
{
    public class GltfMaterialExtractorTests
    {
        ...
        {
            var warnings = GltfMaterialExtractor.Extract(GlbPath, MaterialsDir);
            ...
            Assert.That(wood.GetFloat("_Cull"),
            ...
            Assert.That(glass.GetFloat("_SrcBlend"),
            ...
            var textureImporter = (TextureImporter)AssetImporter.GetAtPath(
                TempDir + "/Textures/box_image_0.png");
            ...
            Assert.That(glass.GetFloat("_ZWrite"), Is.EqualTo(1f),
                "existing materials are reused, not overwritten");
        }
        ...
        public void Reuse_only_extraction_creates_no_materials_folder()
        {
            ...
            using (var fs = File.Create(glbPath))
            {
                GltfWriter.WriteGlb(doc, builder.Finish(), fs);
            }
